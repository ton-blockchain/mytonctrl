...
def test_parse_config_static_nodes():
    config = {
        STATIC_NODE_HEX: {"msg_sender": True, "msg_sender_priority": 5},
        "cc" * 32: {"block_sender": True},
    }
    result = CustomOverlayModule.parse_config("myoverlay", config)
...
def test_parse_config_validators_expands_vset():
    config = {"@validators": True}
    vset = [VALIDATOR_NODE_HEX, "dd" * 32]
    result = CustomOverlayModule.parse_config("dyn", config, vset=vset)
