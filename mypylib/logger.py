...
class LogFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        dt = date_time_library.datetime.fromtimestamp(
            record.created, date_time_library.timezone.utc
        )
        ...
        mode_color = (
            self._LEVEL_COLORS.get(record.levelno, bcolors.UNDERLINE) + bcolors.BOLD
        )
        mode_text = f"{mode_color}{mode_field}{bcolors.ENDC}"
        thread_color = (
            bcolors.ERROR if record.levelno >= logging.ERROR else bcolors.OKGREEN
        ) + bcolors.BOLD
        thread_text = f"{thread_color}{thread_field}{bcolors.ENDC}"
